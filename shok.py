...
def tov():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Тв 10г - 5800 RUB ', callback_data='opl1'))
    key.row(types.InlineKeyboardButton('Работа курьером - 3000 RUB ', callback_data='opl2'))
    key.row(types.InlineKeyboardButton('↩ Назад', callback_data='glm'))
    return key

def tov2():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Тв 10г - 5800 RUB ', callback_data='opl3'))
    key.row(types.InlineKeyboardButton('Работа курьером - 3000 RUB ', callback_data='opl4'))
    key.row(types.InlineKeyboardButton('↩ Назад', callback_data='glm'))
    return key
...
def btc4 ():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Оплата BTC', callback_data='vse4'))
    key.row(types.InlineKeyboardButton('Оплата Qiwi', callback_data='vse44'))
    key.row(types.InlineKeyboardButton(' ↩Назад', callback_data='glm'))
    return key
def btc22 ():
    key = types.InlineKeyboardMarkup()
...
def inlin(c):
    ...
    if c.data=='otk':
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1254\n\nВы приобретаете:\n 📦 Тв 10г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN1\nсумму:\n0,0019 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 0,0019 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse2':
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1212\n\nВы приобретаете:\n 📦 Работа курьером\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN\nсумму:\n0,0011 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервировано место на 30 минут. В течении 30 минут переведите 0,0011 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse3':
        bot.edit_message_text(
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1215\n\nВы приобретаете:\n 📦 Тв 10г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на Qiwi кошелек:\n+79515586641\nсумму:\n5800 RUB,\n❗️ Оплату производить точной суммой RUB и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 5800 RUB на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse22':
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1225\n\nВы приобретаете:\n 📦 Работа курьером\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на Qiwi кошелек:\n+79515586641\nсумму:\n3000 RUB,\n❗️ Оплату производить точной суммой RUB и одним платежом, иначе платеж не будет учтен. Для вас был зарезервировано место на 30 минут. В течении 30 минут переведите 3000 RUB на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse33':
        bot.edit_message_text(
