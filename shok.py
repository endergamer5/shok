...
def tov():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Тв (new) ОПТ ОТ 20гр на заказ- 14000 RUB ', callback_data='opl'))
    key.row(types.InlineKeyboardButton('↩ Назад', callback_data='glm'))
    return key
...
def inlin(c):
    ...
    if c.data=='otk':
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1213\n\nВы приобретаете:\n 📦 Тв (new) ОПТ ОТ 20гр\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN\nсумму:\n0.003391 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 0.003391 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())    
    elif c.data=='check':
        bot.edit_message_text(
