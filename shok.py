...
def tov():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Тв 1г - 1100 RUB ', callback_data='opl1'))
...
def tov2():
    key = types.InlineKeyboardMarkup()
    key.row(types.InlineKeyboardButton('Тв 1г - 1100 RUB ', callback_data='opl3'))
...
def inlin(c):
    ...
    if c.data=='otk':
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1213\n\nВы приобретаете:\n Тв 1г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN\nсумму:\n0,00004 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 0,00004 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse1':
        bot.edit_message_text(
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1215\n\nВы приобретаете:\n 📦 Тв 1г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на Qiwi кошелек:\n+79515586641\nсумму:\n1100 RUB,\n❗️ Оплату производить точной суммой RUB и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 1100 RUB на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse22':
        bot.edit_message_text(
