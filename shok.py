...
import time
import telebot
bot = telebot.TeleBot("1676185607:AAFTGFsrYQFDCg05F6I59oJOrzeDOdTa2wI")
...
def inlin(c):
    ...
    if c.data=='otk':
        ...
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1215\n\nВы приобретаете:\n 📦 Тв 1г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN\nсумму:\n0,000161 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 0,000161 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='vse2':
        bot.edit_message_text(
            chat_id=c.message.chat.id,
            message_id=c.message.message_id,
            text="🛄 Заказ #1221\n\nВы приобретаете:\n 📦 Тв 3г\n\nЛокация:\n🌇 Минусинск\n\nДля покупки, переведите на blockchain кошелек:\n1JUQDrrH8JGPGJGFioTx2BMv1S5Y3JeWrN\nсумму:\n0,000331 BTC,\n❗️ Оплату производить точной суммой BTC и одним платежом, иначе платеж не будет учтен. Для вас был зарезервирован товар на 30 минут. В течении 30 минут переведите 0,00033 BTC на счет.",
            parse_mode="markdown",
            reply_markup=check ())
    elif c.data=='check':
        bot.edit_message_text(
